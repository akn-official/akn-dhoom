...
function WorkContent() {
  ...
  const [editItem, setEditItem] = useState<Partial<Work> | null>(null);
  ...
  const handleSave = async () => {
    ...
    if (coverFile) {
      cover_image_url = await uploadImage(supabase, 'works', coverFile);
    }
    ...
    const payload = {
      ...
      cover_image_url,
      client_name: editItem.client_name || null,
      services_used: editItem.services_used || [],
      results: editItem.results || null,
      is_published: editItem.is_published ?? false,
      display_order: editItem.display_order ?? items.length,
    };

    if (editItem.id) {
      await supabase.from('works').update(payload).eq('id', editItem.id);
    } else {
  ...
  const handleDelete = async (item: Work) => {
    ...
    if (item.cover_image_url) await deleteImage(supabase, 'works', item.cover_image_url);
    await supabase.from('works').delete().eq('id', item.id);
    fetchItems();
  };

  const togglePublish = async (item: Work) => {
    await supabase.from('works').update({ is_published: !item.is_published }).eq('id', item.id);
    fetchItems();
  };
    ...
    <div className="pt-24 sm:pt-32 px-4 sm:px-8 pb-16">
      <div className="max-w-4xl mx-auto">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="font-epilogue text-2xl sm:text-3xl font-bold text-white">Work / Portfolio</h1>
          <Button onClick={() => setEditItem({ is_published: false, display_order: items.length, services_used: [] })} className="ml-auto bg-[#C87A4F] hover:bg-[#A6623D] text-white">
        ...
        {editItem && (
          <div className="mb-8 p-6 rounded-2xl bg-zinc-900/80 border border-zinc-800">
            <h2 className="font-epilogue text-lg font-bold text-white mb-4">{editItem.id ? 'Edit' : 'New'} Work</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div className="space-y-2">
                <Label className="text-zinc-300">Title *</Label>
                <Input value={editItem.title || ''} onChange={(e) => setEditItem({ ...editItem, title: e.target.value, slug: slugify(e.target.value) })} className="bg-zinc-950 border-zinc-800 text-white focus-visible:ring-[#2A8B9D]" />
              </div>
              <div className="space-y-2">
            ...
            <div className="space-y-2 mb-4">
              ...
              <Textarea value={editItem.description || ''} onChange={(e) => setEditItem({ ...editItem, description: e.target.value })} className="bg-zinc-950 border-zinc-800 text-white min-h-[120px] focus-visible:ring-[#2A8B9D]" />
            </div>
            <div className="space-y-2 mb-4">
              <Label className="text-zinc-300">Results / Outcomes</Label>
              <Textarea value={editItem.results || ''} onChange={(e) => setEditItem({ ...editItem, results: e.target.value })} placeholder="e.g. 3x increase in local search visibility..." className="bg-zinc-950 border-zinc-800 text-white min-h-[80px] focus-visible:ring-[#2A8B9D]" />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
              <div className="space-y-2">
                <Label className="text-zinc-300">Services Used (comma separated)</Label>
                <Input value={(editItem.services_used || []).join(', ')} onChange={(e) => setEditItem({ ...editItem, services_used: e.target.value.split(',').map(s => s.trim()).filter(Boolean) })} placeholder="Web Design, SEO, GMB" className="bg-zinc-950 border-zinc-800 text-white focus-visible:ring-[#2A8B9D]" />
              </div>
              <div className="space-y-2">
        ...
        <div className="space-y-3">
          {items.length === 0 && (
            <p className="text-zinc-500 text-center py-12">No portfolio items yet. Add your first one above.</p>
          )}
          {items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 p-4 rounded-xl bg-zinc-900/50 border border-zinc-800">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {item.client_name && <span className="text-zinc-500 text-xs">— {item.client_name}</span>}
                  {item.is_published ? (
